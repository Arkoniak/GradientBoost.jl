...
function test_loss(lf::LossFunction, 
  ...
  for i = 1:size(y_examples, 1)
    @fact loss(lf, y_examples[i], y_pred_examples[i]) --> roughly(expected[i], atol = 1e-5)
  end
end
...
function test_minimizing_scalar(lf::LossFunction, 
  ...
  for i = 1:size(y_examples, 1)
    @fact minimizing_scalar(lf, y_examples[i]) --> roughly(expected[i], atol = 1e-5)
  end
end
