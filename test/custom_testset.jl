module CustomTS
using Base.Test: Broken, Pass, Fail, Error, AbstractTestSet, TestSetException
using Base.Test: get_testset_depth, get_testset, scrub_backtrace
import Base.Test: record, finish

export CustomTestSet
"""
    CustomTestSet
If using the CustomTestSet, the test results will be recorded. If there
are any `Fail`s or `Error`s, an exception will be thrown only at the end,
along with a summary of the test results.
"""
mutable struct CustomTestSet <: AbstractTestSet
    description::AbstractString
    results::Vector
    n_passed::Int
    anynonpass::Bool
end
CustomTestSet(desc) = CustomTestSet(desc, [], 0, false)

# For a broken result, simply store the result
record(ts::CustomTestSet, t::Broken) = (push!(ts.results, t); t)
# For a passed result, do not store the result since it uses a lot of memory
record(ts::CustomTestSet, t::Pass) = (ts.n_passed += 1; t)

# For the other result types, immediately print the error message
# but do not terminate. Print a backtrace.
function record(ts::CustomTestSet, t::Union{Fail, Error})
    if myid() == 1
...
# When a CustomTestSet finishes, it records itself to its parent
# testset, if there is one. This allows for recursive printing of
# the results at the end of the tests
record(ts::CustomTestSet, t::AbstractTestSet) = push!(ts.results, t)

function print_test_errors(ts::CustomTestSet)
    for t in ts.results
        if (isa(t, Error) || isa(t, Fail)) && myid() == 1
            println("Error in testset $(ts.description):")
            Base.show(STDOUT,t)
            println()
        elseif isa(t, CustomTestSet)
            print_test_errors(t)
        end
    end
end

function print_short_test_results(ts::CustomTestSet)
    ...
    msg = ">"^depth*msg
    print(msg)
    msg = @sprintf("Pass: %s, Fail: %s, Error: %s, Broken: %s\n", total_pass, total_fail, total_error, total_broken)
    msg = " "^depth*"  "*msg
    print(msg)
end

function print_test_results(ts::CustomTestSet, depth_pad=0)
    # Calculate the overall number for each type so each of
    # the test result types are aligned
    passes, fails, errors, broken, c_passes, c_fails, c_errors, c_broken = get_test_counts(ts)
    total_pass   = passes + c_passes
...
function get_alignment(ts::CustomTestSet, depth::Int)
...
function filter_errors(ts::CustomTestSet)
    efs = []
    for t in ts.results
        if isa(t, CustomTestSet)
...
function get_test_counts(ts::CustomTestSet)
...
function print_counts(ts::CustomTestSet, depth, align,
